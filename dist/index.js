...
function ImageEditorCore(props) {
    const { minimumCropDimensions = { width: 100, height: 100 }, fixedAspectRatio = 0.66666666666, onEditingCancel, onEditingComplete, imageUri = null, processingComponent, editorOptions, aspectRationOptions, freeCrop } = props;
    ...
    const [defaultAspectRatio, setDefaultAspectRatio] = useState(fixedAspectRatio);
    const [aspectRatioOptions, setAspectRatioOptions] = useState(aspectRationOptions ?? []);
    ...
    return (React.createElement(EditorContext.Provider, { value: {
        ...
        React.createElement(StatusBar, { hidden: true, animated: true, backgroundColor: options.backgroundColor, barStyle: 'light-content' }),
        React.createElement(ImageEditorView, { processingComponent: processingComponent })));
}
export function ImageEditorView({ processingComponent, freeCrop }) {
    ...
    return (React.createElement(React.Fragment, null,
        ready && (React.createElement(View, { style: [styles.container, {
            ...
            controlBar?.position === 'bottom' && React.createElement(ControlBar, null))),
        processing && React.createElement(Processing, { customComponent: processingComponent })));
}
export function ImageEditor({ isVisible, ...props }) {
    return (React.createElement(Modal, { visible: isVisible, style: styles.modalContainer },
